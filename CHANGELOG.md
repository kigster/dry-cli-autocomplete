...
- The bash script now completes values declared on an option. `mycli fmt --format`
  then TAB offered the command list again instead of the values `--format` accepts;
  zsh had always emitted them, so only the bash side was dropping them.
- The bash script now offers values declared on a positional argument at that
  position, alongside the node's flags and subcommands.
